...
function initRPC() {
	 ...
	 * Returns wallet balance(stable and pending) without commissions earned from headers and witnessing.
	 *
	 * @return {"base":{"stable":{Integer},"pending":{Integer}}} balance include third asset
	 */
	server.expose('getwalletbalance', function(args, opt, cb) {
		let start_time = Date.now();
		Wallet.readBalance(wallet_id, function(assocBalances) {
			console.log('getwalletbalance took '+(Date.now()-start_time)+'ms');
	...
	server.expose('sendtomultiaddress', function(args, opt, cb) {
		try{
			...
			if (from_address && arrAddressesAndAmount) {
                if(asset) {
                    createMultiToAddressPaymentOfThirdAsset(from_address, arrAddressesAndAmount, asset, function(err){ // send third asset
    ...
    function createMultiToAddressPaymentOfThirdAsset(from_address, arrAddressesAndAmount, asset, cb1){
		function onError(err){
			cb1(err);
		}
		var callbacks = composer.getSavingCallbacks({
		...
		composer.composeJoint({paying_addresses: [from_address], outputs: arrOutputs, signer: headlessWallet.signer, asset: asset, callbacks: callbacks});
    }
