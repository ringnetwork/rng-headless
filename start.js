	...
	readKeys(function(mnemonic_phrase, passphrase, deviceTempPrivKey, devicePrevTempPrivKey){
		...
		readSingleWallet(function(wallet){
			...
			db.query("SELECT 1 FROM extended_pubkeys WHERE device_address=?", [my_device_address], function(rows){
				...
				require('rng-core/wallet/wallet.js'); // we don't need any of its functions but it listens for hub/* messages
				...
				if (conf.bLight){
					var light_wallet = require('rng-core/wallet/light_wallet.js');
...
function handleText(from_address, text){
	...
	var walletDefinedByKeys = require('rng-core/wallet/wallet_defined_by_keys.js');
	var device = require('rng-core/device.js');
	switch(command){
		case 'address':
			if (conf.bSingleAddress)
